...
enum ZLIB_VERSION = "1.2.8";
enum ZLIB_VERNUM = 0x1280;
enum ZLIB_VER_MAJOR = 1;
enum ZLIB_VER_MINOR = 2;
enum ZLIB_VER_REVISION = 8;
...
alias void* function (void*, uint, uint) alloc_func;
alias void function (void*, void*) free_func;
...
/* next output byte should be put there */
/* remaining free space at next_out */
/* total number of bytes output so far */ /* last error message, NULL if no error */ /* not visible by applications */
...
int inflateBack (z_streamp strm, in_func in_, void* in_desc, out_func out_, void* out_desc);
/*
     inflateBack() does a raw inflate with a single call using a call-back
   interface for input and output.  This is potentially more efficient than
   inflate() for file i/o applications, in that it avoids copying between the
...
alias gzFile_s* gzFile; /* semi-opaque gzip file descriptor */
 ...
 * and the compiler's view of z_stream:
 */
int deflateInit_ (z_streamp strm, int level, const(char)* version_, int stream_size);
int inflateInit_ (z_streamp strm, const(char)* version_, int stream_size);
int deflateInit2_ (z_streamp strm, int level, int method, int windowBits, int memLevel, int strategy, const(char)* version_, int stream_size);
int inflateInit2_ (z_streamp strm, int windowBits, const(char)* version_, int stream_size);
int inflateBackInit_ (z_streamp strm, int windowBits, ubyte* window, const(char)* version_, int stream_size);
...
c_long gzseek (gzFile, c_long, int);
c_long gztell (gzFile);
c_long gzoffset (gzFile);
uLong adler32_combine (uLong, uLong, c_long);
uLong crc32_combine (uLong, uLong, c_long);
